...
func Build(cfgs *SpritePackCfg) ([]Sprite, []PaletteRGBA, error) {
	...
	for _, cfg := range cfgs.Files {
		...
		for _, s := range spritesCfg.Sprites {
			...
			alphaFormat, err := AlphaFormatFromInt(s.Alpha)
			if err != nil {
				return nil, nil, err
			}
			...
			case FMT_PIXEL_RGB565:
				...
				default:
					return nil, nil, fmt.Errorf("unsupported alpha format for RGB565: %s", alphaFormat)
				}
			...
			default:
				return nil, nil, fmt.Errorf("unsupported format: %s", pixelFormat)
			}
