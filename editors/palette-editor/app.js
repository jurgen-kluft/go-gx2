...
const elements = {
  ...
  saveRgbaPngButton: document.getElementById('saveRgbaPngButton'),
...
function isCanvasMode() {
  return state.viewMode === 'preview' || state.viewMode === 'alpha';
}
...
function setViewMode(mode) {
  state.viewMode = mode === 'preview' || mode === 'alpha' ? mode : 'editor';
  const canvasMode = isCanvasMode();
  const modeLabel = state.viewMode === 'alpha'
    ? 'Alpha'
    : (state.viewMode === 'preview' ? 'Image' : 'Palette');

  elements.editorWorkspace.classList.toggle('hidden', canvasMode);
  elements.previewWorkspace.classList.toggle('hidden', !canvasMode);
  elements.viewModeButton.textContent = `View: ${modeLabel}`;
  elements.viewModeButton.title = `Currently ${modeLabel} View`;
  elements.viewModeButton.classList.toggle('active', canvasMode);
  elements.canvas.setAttribute(
    'aria-label',
    state.viewMode === 'alpha' ? 'Source alpha channel preview' : 'Reduced-color image preview',
  );
  elements.zoomSelect.disabled = !hasImage() || !canvasMode;
  elements.zoomOutButton.disabled = !hasImage() || !canvasMode;
  elements.zoomInButton.disabled = !hasImage() || !canvasMode;
  elements.zoomResetButton.disabled = !hasImage() || !canvasMode;
...
function updateControls() {
  ...
  elements.saveRgbaPngButton.disabled = !hasPalette;
  elements.zoomSelect.disabled = !active || !canvasMode;
  elements.zoomOutButton.disabled = !active || !canvasMode;
  elements.zoomInButton.disabled = !active || !canvasMode;
  elements.zoomResetButton.disabled = !active || !canvasMode;
...
function renderAlphaPreview() {
  state.previewImageData = renderAlphaToImageData(state.sourceImageData);
  sourceCtx.putImageData(state.previewImageData, 0, 0);
  canvasCtx.setTransform(1, 0, 0, 1, 0, 0);
  canvasCtx.clearRect(0, 0, elements.canvas.width, elements.canvas.height);
  canvasCtx.imageSmoothingEnabled = false;
...
function saveRgbaPng() {
  ...
  sourceCanvas.toBlob((blob) => {
    if (!blob) {
      setStatus('RGBA PNG export failed.', 'bad');
      return;
    }

    const url = URL.createObjectURL(blob);
    const anchor = document.createElement('a');
    const baseName = (state.imageName || 'rgba').replace(/\.[^.]+$/, '');
    anchor.href = url;
    anchor.download = `${baseName}.rgba.png`;
    document.body.append(anchor);
    anchor.click();
    anchor.remove();
    URL.revokeObjectURL(url);
...
function registerEvents() {
  ...
  elements.saveRgbaPngButton.addEventListener('click', saveRgbaPng);
