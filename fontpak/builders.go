...
func Build(cfg *FontPackCfg) ([]Font, []FontInfo, error) {
	...
	for _, fontCfg := range cfg.Fonts {
		...
		if numChars > cap(glyphRunes) {
			return nil, nil, fmt.Errorf("font file \"%q\" has too many chars in char map (max %d)", fontCfg.File, cap(glyphRunes))
		}
...
func buildTTFFont(fontPath string, ops FontOptions, name string, glyphRunes []rune) (*builtFont, error) {
...
func buildBDFFont(fontPath string, ops FontOptions, name string, glyphRunes []rune) (*builtFont, error) {
